...
**1. 원본 정보 최대한 활용 (매우 중요!)**
❗❗ 원본 이력서에 있는 모든 정보를 반드시 추출하여 사용하십시오 ❗❗

- 회사명, 직책, 재직기간 → 원본에서 찾아서 기재
- 학력 (대학명, 전공, 졸업일) → 원본에 있으면 반드시 기재
- 자격증, 어학, 수상 → 원본에 있으면 모두 포함
- 프로젝트 경험 → 원본에 언급된 것 모두 포함
- 정량 수치(매출, 인원, 처리량) → 원본에 있는 것 모두 활용

❌ 절대 금지:
- 원본에 있는 정보를 플레이스홀더([대학교], [전공])로 대체
- 원본에 명시된 정보를 누락
- 없는 성과/경험 창작

✅ 원칙:
- 원본에 있는 정보 → 100% 활용
- 원본에 없는 정보 → 플레이스홀더 또는 생략
- 애매한 정보 → 원본 문구 그대로 사용
