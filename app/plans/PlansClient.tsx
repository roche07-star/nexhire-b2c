...
const faqData = {
  JOBSEEKER: [
    ...
    {
      q: '환불 정책은 어떻게 되나요?',
      a: '구매 후 7일 이내, 서비스를 5회 미만 사용하신 경우 전액 환불이 가능합니다. 플랜 만료 시 미사용 횟수는 소멸되며 환불되지 않습니다.'
    },
    {
      q: '플랜 만료 후 미사용 횟수는 어떻게 되나요?',
      a: '플랜 만료 시 남은 횟수는 자동 소멸됩니다. 재가입 시 해당 플랜의 기본 횟수가 제공되며, 이전 미사용 횟수는 이월되지 않습니다. 보너스는 프로모션 기간에만 별도 제공됩니다.'
    },
    {
      q: '쿠폰과 플랜 횟수는 어떤 순서로 사용되나요?',
      a: '플랜 횟수를 먼저 사용하며, 쿠폰은 플랜 횟수 소진 후 자동으로 사용됩니다. 쿠폰은 유효기간(3개월)이 있으므로, 만료 전에 사용하시기 바랍니다.'
    },
    {
      q: '플랜이 만료되어도 쿠폰은 사용할 수 있나요?',
      a: '네, 플랜이 FREE로 다운그레이드되어도 보유 중인 쿠폰은 유효기간 내에 계속 사용할 수 있습니다. FREE 한도 소진 후에도 쿠폰으로 서비스를 이용하실 수 있습니다.'
    },
  ],
  HEADHUNTER: [
    {
...
export default function PlansClient({ userEmail, userType, currentPlan, isSuperAdminOrManager }: PlansClientProps) {
  ...
  // KCP Test 계정은 구직자/헤드헌터 플랜 모두 선택 가능
  ...
  return (
    <main className="zero-nav-spacing" style={{
