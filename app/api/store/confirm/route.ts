...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!secretKey) {
      ...
      return NextResponse.json({ error: '결제 처리 중 오류가 발생했습니다. 고객센터로 문의해주세요.' }, { status: 500 })
    }
    ...
    try {
      await supabase.from('notifications').insert({
        ...
        icon: '✅',
        title: '구매 완료',
        message: `${productName} 쿠폰이 발급되었습니다.`,
        link: '/my-info',
        is_read: false,
      })
      console.log('[store/confirm] 알림 생성 완료:', session.user.email)
    } catch (notifError) {
      console.error('[store/confirm] 알림 생성 실패:', notifError)
