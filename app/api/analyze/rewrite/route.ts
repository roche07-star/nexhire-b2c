...
**1. 핵심역량 / 업무상 강점 항목 수정**
⚠️ **기존 이력서 내용 배제**: 기존 이력서의 핵심역량/업무상 강점 내용을 참고하지 말고, 아래 지침만으로 새롭게 작성하십시오.

${proposal?.strengths ? `
**제안서 강점 기반 작성**:
${proposal.strengths.map((s: string, i: number) => `${i + 1}. ${s}`).join('\n')}

위 제안서 핵심 강점을 그대로 사용하여 핵심역량/업무상 강점 섹션을 작성하십시오.
` : `
**JD 분석 기반 작성**:
- JD 필수 요건 키워드를 첫 번째 항목으로 전면 배치: ${toArr(jd.matching_points).slice(0, 3).join(', ')}
- 위 matching_points를 기반으로 핵심역량/업무상 강점을 구체적으로 작성
