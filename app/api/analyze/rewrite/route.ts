...
function buildSectionPrompts(resumeText: string, jd: JDContext | null): { system: string; user: string } {
  ...
  const jdAggressiveRules = jd ? `
[🎯 JD 최적화 핵심 원칙 — 모든 섹션에 최우선 적용]

**중요: 원본은 참고 자료일 뿐입니다. JD 요구사항에 맞게 적극 재작성하십시오.**

1. **핵심역량 / 업무상 강점 섹션**
   - matching_points를 핵심 내용으로 재구성 (${toArr(jd.matching_points).slice(0, 3).join(' / ')})
   - JD 포지션이 요구하는 역량 중심으로 완전히 재작성
   - 원본 내용 중 JD와 무관한 내용은 과감히 삭제
   - 수치/성과는 JD 요구사항과 연결지어 재배치
   - 예시: "10년 경력" → "${jd.position || '해당 포지션'}에 필요한 10년 핵심 경험"

2. **이직사유 / 세부경력사항 섹션**
   - "${jd.company}"에 지원하는 이유 중심으로 완전 재작성
   - 현재 커리어 → ${jd.company} 포지션 적합성 → 미래 기여 방향 구조
   - gaps를 긍정적 성장 기회로 전환 (${toArr(jd.gaps).slice(0, 2).join(', ')})
   - 경력사항은 JD 요구 역량 순서대로 재배치

3. **전략적 키워드 배치**
   - pitch_points를 자연스럽게 녹임: ${toArr(jd.pitch_points).join(' / ')}
   - JD 적합도 ${jd.fit_score}점의 근거가 명확히 드러나도록 작성
   - matching_points의 키워드를 섹션 제목/첫 문장에 배치

4. **자기소개서 섹션 (완전 재작성)**
   - "${jd.company}"를 2회 이상 자연스럽게 언급
   - STAR 구조: 상황(${jd.company} 포지션 적합성) → 역할(핵심 강점) → 행동(구체적 경험) → 결과(기대 기여)
   - 첫 문장: ${jd.company} 포지션과 후보자의 강력한 연결고리
   - AI 표현 완전 금지: "귀사", "시너지", "열정적으로", "다양한 경험"

5. **원본 의존도 최소화**
   - 원본 문장을 그대로 복사하지 마십시오
   - 원본의 사실/수치만 활용하고, 표현은 JD 중심으로 재작성
   - "원본 그대로" 원칙은 인적사항/학력/자격증에만 적용` : ''
...
[새 양식 작성 원칙]
1. 원본 이력서의 사실/수치만 활용하고, 표현은 JD 중심으로 재작성
...
6. **핵심역량/업무상 강점 단락**: matching_points를 중심으로 완전 재작성 (${toArr(jd.matching_points).slice(0, 3).join(' / ')})
   - 원본 문장을 그대로 복사하지 말고, JD 요구사항에 맞게 재구성
   - JD 포지션이 요구하는 역량이 명확히 드러나도록 작성
7. **경력사항 단락**: JD 요구 역량 순서대로 재배치, gaps는 성장 기회로 전환
8. **이직사유 단락**: ${jd.company} 지원 동기 중심으로 완전 재작성
9. pitch_points 키워드를 자연스럽게 녹임: ${toArr(jd.pitch_points).join(' / ')}` : ''}
10. 가운데점 "/" 사용 금지, 구분은 "/" 또는 "," 사용
11. 각 단락 길이를 양식 단락 길이와 비슷하게 유지합니다${jd ? `

[🎯 자기소개서 / 지원사유 / 지원동기 / 포부 단락 — 완전 재작성 필수]
해당 단락 위치가 감지되면 원본 이력서 내용을 참고만 하고 JD 중심으로 완전 재작성:
- "${jd.company}"를 본문에 2회 이상 자연스럽게 명시
- 어필 전략 키워드: ${toArr(jd.pitch_points).join(' / ')}
- AI 표현 완전 금지: "귀사", "시너지", "다양한 경험", "열정적으로"
- 지원사유 구조: 커리어 방향 → ${jd.company} 접점 → 기여 방향
- 자기소개 구조: STAR (상황→역할→행동→결과) + 인상적인 첫 문장
- JD 적합도 ${jd.fit_score}점의 근거가 명확히 드러나도록 작성
- "/" → "/" 전면 교체
- **중요**: 원본 자기소개서 문장을 그대로 복사하지 말고, JD에 맞게 새로 작성` : ''}`
