    ...
    ">
      ...
      ">
        ...
        ">
          ...
          <strong style="color: #2563eb; font-weight: 600;">Jobizic</strong>
        </div>
      </div>
    </div>`
...
항목 2 — 자기소개서
목적: 이력서 사실을 기반으로 하되, 그 뒤의 스토리와 맥락을 풀어내는 것.
직무 역량 + 성격 + 실전 경험이 유기적으로 연결된 서술.

【핵심 원칙: 스토리텔링】
이력서의 "무엇을 했다"는 사실입니다.
자기소개서는 "왜 그렇게 했는지", "어떤 과정이었는지", "무엇을 배웠는지"를 풀어내는 것입니다.

예시:
- 이력서: "MAU 30% 증가"
- 자기소개서: "초반 가설이 틀렸음을 데이터로 깨달은 순간, 팀 전체의 방향을 바꾸는 용기가 필요했습니다. 그 결정이 결국 MAU 30% 증가로 이어졌습니다."

【작성 방향】
다음 소재 2~3개를 유기적으로 연결:
① 이력서 성과의 **뒷이야기** — 그 결과를 만들기까지의 과정, 시행착오, 의사결정 순간
② 업무 철학/습관 — 구체적 상황에서 발휘된 일하는 방식
   "___상황에서 ___방식으로 접근해 ___결과를 냈습니다" 구조
③ 실패 또는 배움의 순간 — STAR 구조 (상황→역할→행동→결과/배움)
   ※ 성공담보다 **실패 후 배운 점**이 더 진정성 있음
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      if (plan === 'PRO' || plan === 'EXPERT') {
        ...
        const protectionKey = `paid-protection:rewrite:${email}`
...
3. 업무상 강점 및 핵심 역량
   형식:
   ■ 대제목 (주요 강점)
     - 세부 설명 1 (구체적 수치 포함, 각 항목은 줄바꿈으로 구분)
     - 세부 설명 2
     - 세부 설명 3
   ■ 대제목 (주요 강점)
     - 소항목 1
   ...
   예시:
   ■ 글로벌 규제 및 위생허가 실무 전문성
     - 중국/미국/EU/일본 등 주요 시장 화장품 법규 및 등록 프로세스 숙지
     - 중국 NMPA 등록 300건 이상 성공 경험 (승인율 95%)
     - 글로벌 규제 변경사항 모니터링 및 대응 전략 수립

   ※ 이 섹션에만 ■ 와 - 불렛 사용 (다른 섹션은 불렛 금지)
   ※ 각 세부 항목은 반드시 줄바꿈으로 구분, 한 줄당 하나의 내용만 작성

4. 경력 사항(총 경력: OO년 OO개월)
   ⚠️ 중요: 섹션 제목에 반드시 "(총 경력: OO년 OO개월)" 형식으로 포함할 것
   형식: |재직기간|회사명|직무|소재지|
   예시: |2016.11 ~ 2024.07 (5개월)|애경산업|AGE20'S BM|서울|
   ⚠️ 재직기간에 반드시 개월수 포함, "재직중" "퇴직" 표시 절대 사용 금지

5. 세부 경력사항
   형식:
   【 회사명 YYYY년 MM월 ~ YYYY년 MM월 (OO년 OO개월) 직급 】
   ⚠️ 기간에 반드시 개월수 포함, "재직 중" "퇴직" 표시 절대 사용 금지
   회사 소개, 매출액, 인원
   [담당업무] (각 담당업무는 줄바꿈으로 구분, 한 줄당 하나의 업무만 작성)
   - 담당업무1
   - 담당업무2
   [상세업무] (각 업무는 줄바꿈으로 구분, 한 줄당 하나의 업무만 작성)
   - 업무 항목1
   - 업무 항목2
   - 업무 항목3
   [성과] (각 성과는 줄바꿈으로 구분, 한 줄당 하나의 성과만 작성)
   - 성과1
   - 성과2
   [이직사유] (각 회사별 필수, 한 문장으로 작성, 불렛 포인트 사용 금지)
   - 직전 회사 성과 강조 → 자연스러운 발전 방향으로 한 문장 연결
   - 예: "A사 제조 자동화 경험 축적 후 B사에서 IoT 융합 솔루션으로 기술 확장"
   - 긍정 키워드: "전문성 심화", "기술 확장", "역할 확대", "도메인 다각화"
   - 현재 재직중인 회사는 이직사유 생략
   ⚠️ 중요: 이직사유는 반드시 한 줄로 작성, 여러 줄이나 불렛 포인트로 나누지 말 것
...
4. 불렛 기호(•, -, *, 1. 등) 사용 금지
5. 줄 맞춤: 각 항목은 새 줄(\\n)로 구분, 들여쓰기 일관성 유지
6. 마크다운 문법 절대 사용 금지: **bold**, *italic* 등 마크다운 표시 사용 금지
7. AI 스러운 표현 절대 금지:
