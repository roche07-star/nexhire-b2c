...
const proBasicTool: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      summary: { type: 'string', description: '지원자에 대한 전체 요약. 반드시 다음을 포함: 1) 커리어 패턴(성장형/전환형/순환형/분산형), 2) 이직 동기 추정, 3) 핵심 특징. 2-3문장으로 작성.' },
...
반드시 다음을 포함한 2-3문장으로 작성:
1. 커리어 패턴: "이 후보자의 커리어는 [성장형/전환형/순환형/분산형]이며, [이유]"
2. 이직 동기: 이력서 패턴에서 역추정
3. 핵심 특징: 가장 두드러지는 강점 또는 리스크 1가지

[출력 규칙] 빈 말("다양한 경험", "훌륭한 경력") 절대 금지. 날짜·경력 계산 오류 금지. summary는 반드시 2-3문장으로 작성.
