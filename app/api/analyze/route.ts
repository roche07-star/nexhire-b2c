...
아래 후보자의 **실제 경력과 역량**을 정확히 분석하여, **구직자 관점에서 실행 가능한** 3가지 커리어 방향을 제시하십시오.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 후보자 프로필
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• 현재 직무: ${basicInput.job_title ?? '미상'}
• 총 경력: ${basicInput.total_experience_years ?? 0}년
• 학력: ${basicInput.education ?? '미상'}
• 현재 연봉: ${basicInput.current_salary || '정보 없음'}
...
**⚠️ 연봉 추정 기준 (매우 중요!)**
${basicInput.current_salary
  ? `현재 연봉 정보: ${basicInput.current_salary}
→ 이 연봉을 기준으로 BASELINE/RECOMMENDED/STRETCH 연봉 밴드를 계산하십시오.`
  : `현재 연봉 정보 없음 → 총 경력 ${basicInput.total_experience_years ?? 0}년, 직무 "${basicInput.job_title ?? '미상'}", 학력 "${basicInput.education ?? '미상'}"을 기반으로 한국 시장 평균 연봉을 추정한 뒤, 이를 기준으로 연봉 밴드를 계산하십시오.

[한국 시장 평균 연봉 참고 기준]
- 1-3년차: 3,000만원~4,500만원 (신입~주임급)
- 4-7년차: 4,500만원~6,500만원 (대리~과장급)
- 8-12년차: 6,500만원~9,000만원 (과장~차장급)
- 13년차 이상: 9,000만원~1억 2천만원+ (차장~부장급)
※ 학력(석박사), 직무(개발자/전문직), 산업(IT/금융)에 따라 ±20% 조정`}

**1. BASELINE (현재 경로 유지)**
- 현재 직무를 그대로 이어가는 가장 안정적인 경로
- 같은 직무, 더 나은 회사/조건으로 이동
- 리스크: 낮음 / 실행 난이도: 낮음
- 연봉: 현재(또는 추정) ± 10% 범위

**2. RECOMMENDED (추천 경로) ★★★ 가장 중요!**
- **후보자의 강점을 최대한 활용**하면서도 **지금 당장 지원 가능한** 포지션
- 단순히 높은 연봉이 아니라, **실제 합격 가능성이 높은** 현실적 제안
- 현재 경력/역량에서 **자연스럽게 연결되는** 직무 (큰 전환 아님)
- 리스크: 중간 / 실행 난이도: 중간
- 연봉: 현재(또는 추정) 대비 +10~30%

**3. STRETCH (도전 경로)**
- 2~3년 준비 시 도달 가능한 고성장 경로
- 시장 희소성이 높지만, 후보자가 **충분히 준비하면** 도달 가능
- 현재 역량 + 추가 학습/경험으로 연결 가능한 경로
- 리스크: 높음 / 실행 난이도: 높음
- 연봉: 현재(또는 추정) 대비 +30~50%
...
❌ **절대 금지:**
- 추상적인 조언 ("네트워킹을 강화하세요", "역량을 키우세요")
- 현실성 없는 고연봉 제시 (현재 3천만원인데 BASELINE이 6천만원)
- 후보자 역량과 무관한 트렌디한 직무 제안 ("AI 전문가", "메타버스 기획자")
- 막연한 직무명 ("Senior Developer", "PM", "전략 기획자")

✅ **반드시 포함:**
- **구체적인 직무명** (예: "B2B SaaS 백엔드 개발자", "핀테크 프로덕트 오너")
- **실전 조언 2개**: 6개월 내 실행 가능한 구체적 액션 아이템
  예) "Docker/k8s 실무 경험 3개월 내 확보 (사내 프로젝트 또는 오픈소스 기여)"
  예) "현재 회사에서 주니어 개발자 1명 멘토링 경험 확보 (리딩 역량 증명)"
- **한국 채용 시장 기준 연봉 밴드** (Wanted, Rocketpunch 기준)
- **1년 뒤/3년 뒤/5년 뒤 연봉 밴드**: 현실적인 성장 곡선
...
- title: 현실적인 직무명
- salary_range: **이력서에 현재 연봉(current_salary)이 명시되어 있으면 이를 기준으로 ±10% 범위 계산. 없으면 총 경력, 직무, 학력을 기반으로 한국 시장 평균 연봉 추정** (예: 4,500만원~6,500만원)
  [한국 시장 평균 연봉 참고: 1-3년차 3,000~4,500만원, 4-7년차 4,500~6,500만원, 8-12년차 6,500~9,000만원, 13년차+ 9,000~12,000만원, 학력/직무/산업 고려 ±20%]
- salary_bands: 1년 뒤/3년 뒤/5년 뒤/7년 뒤+ 연봉 밴드 4개 (현재 또는 추정 연봉 기준 성장 곡선, min/max 만원 단위)
