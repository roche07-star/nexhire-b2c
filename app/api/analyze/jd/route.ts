...
**⚠️⚠️ 중요: "부족한 점, 리스크" 분석은 필수입니다 ⚠️⚠️**
- gaps 배열은 반드시 2-3개를 포함해야 합니다. "특별한 리스크가 감지되지 않았습니다" 같은 답변은 절대 금지.
- 어떤 후보자든 반드시 부족한 점이나 리스크가 있습니다: 경력 부족, 특정 기술 미보유, 연봉 기대치 괴리, 이직 횟수, 도메인 경험 부족, 학력, 나이, 근무지 거리, 경력 공백, 자격증 미보유 등.
- 리스크는 심각도 순으로 정렬하고, 가장 치명적인 것을 최상단에 배치하세요.
- 최소 2-3개 이상의 구체적인 리스크를 반드시 명시하세요. "없음", "특이사항 없음" 같은 답변은 금지입니다.
...
⚠️ 경력 공백이 1년 이상인 경우 반드시 gaps 또는 pitch_points에 언급하고 대응 방안 제시

[fit_score 점수 기준] ⚠️ 반드시 준수
- 85-100점: 우수 매칭. 필수 스킬 60% 이상 + 우대 스킬 25% 이상 → APPLY
- 70-84점: 양호 매칭. 필수 스킬 40% 이상 + 우대 스킬 10% 이상 → CONSIDER
- 55-69점: 보통 매칭. 필수 스킬 20% 이상 → CONSIDER
- 40-54점: 부족 매칭. 필수 스킬 10% 이상 → SKIP
- 0-39점: 부적합. 필수 스킬 거의 없음 → SKIP

⚠️ 중요: 실제 헤드헌팅 시장에서는 필수 스킬 60-70% 매칭이면 "우수" 후보자입니다.
완벽한 100% 매칭은 거의 존재하지 않으며, 80% 이상 매칭되는 후보자도 드뭅니다.
후보자의 잠재력, 학습 능력, 유사 경험, 전이 가능 스킬, 성장 가능성을 적극적으로 평가하여 점수에 반영하십시오.
특히 경력이 있고 기본 역량이 검증된 후보자라면 새로운 스킬 습득은 충분히 가능하므로 긍정적으로 평가하십시오.
...
예시:
- 필수 스킬 5개 중 4개 충족 (80%) + 우대 4개 중 2개 충족 (50%) → 반드시 90-95점 (최상위권)
- 필수 스킬 5개 중 3개 충족 (60%) + 우대 4개 중 1개 충족 (25%) → 반드시 82-88점
- 필수 스킬 5개 중 2개 충족 (40%) + 우대 1개 충족 → 70-75점 정도로 평가
- 필수 스킬 5개 중 2개 충족 (40%) → 65-70점 정도로 평가
...
- matching_points: **[필수 작성]** 필수✅ + 우대✅ + 숨은요구✅ 항목 (구체적 근거 포함, 정확히 3개 필수)
- gaps: **[필수 작성]** 필수❌/△ + 예상 클라이언트 우려 사항 (정확히 2-3개, 솔직하게, 빈 배열 절대 금지). ⚠️⚠️ 모든 후보자는 반드시 부족한 점이 있습니다! 경력 부족, 특정 스킬 미보유, 연봉 괴리, 이직 횟수, 도메인 경험 부족, 학력, 경력 공백 등 최소 2-3개의 구체적 리스크를 반드시 작성하세요. "특이사항 없음", "리스크 없음" 같은 답변은 절대 금지!
- pitch_points: **[필수 작성]** ① 클라이언트 제안 포지셔닝 전략 ② 서류/면접 핵심 어필 포인트 ③ 예상 우려 대응 방안 ④ 제안 전 후보자에게 반드시 확인할 사항 (정확히 3-4개, 실전적으로, 빈 배열 절대 금지)
