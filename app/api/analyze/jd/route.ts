...
- 85-100점: 우수 매칭. 필수 스킬 75% 이상 + 우대 스킬 40% 이상 → APPLY
- 70-84점: 양호 매칭. 필수 스킬 55% 이상 + 우대 스킬 25% 이상 → CONSIDER
- 55-69점: 보통 매칭. 필수 스킬 35% 이상 → CONSIDER
- 40-54점: 부족 매칭. 필수 스킬 15% 이상 → SKIP
- 0-39점: 부적합. 필수 스킬 거의 없음 → SKIP

⚠️ 중요: 실제 헤드헌팅 시장에서는 필수 스킬 65-75% 매칭이면 "우수" 후보자입니다.
완벽한 100% 매칭은 거의 존재하지 않으므로, 점수를 과도하게 깎지 마십시오.
후보자의 잠재력과 학습 능력도 긍정적으로 평가하여 점수에 반영하십시오.
...
예시:
- 필수 스킬 5개 중 4개 충족 (80%) + 우대 4개 중 2개 충족 (50%) → 반드시 85-100점 (상위권)
- 필수 스킬 5개 중 3개 충족 (60%) + 우대 4개 중 1개 충족 (25%) → 반드시 70-84점
- 필수 스킬 5개 중 2개 충족 (40%) → 55-69점 정도로 평가
