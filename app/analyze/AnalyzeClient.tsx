...
export default function AnalyzeClient({ initialIsPro, initialIsExpert, userEmail, userType }: { initialIsPro: boolean; initialIsExpert?: boolean; userEmail: string | null; userType?: string | null }) {
  ...
  useEffect(() => {
    if (activeMenu === 'interview' && analysisList === null && !savedListLoading) {
...
<body>
  ...
  <div class="container">
    ...
    <div class="footer">
      Generated by Jobizic, Resume Generator
    </div>
  </div>
</body>
</html>
        ...
        <div className="analyze-main">
          <div className="analyze-container">
              ...
              {(activeMenu === 'saved' || activeMenu === 'upload') && result ? (
                ...
                <>
                  <h1 className="analyze-title">
                    {activeMenu === 'jd' ? 'JD 적합도 분석' : activeMenu === 'saved' ? (userType === 'HEADHUNTER' ? '분석 결과' : '분석 & 이력서 재생성') : activeMenu === 'rewrite' ? '이력서 생성' : activeMenu === 'interview' ? '면접 가이드' : '이력서 분석'}
                  </h1>
                  {activeMenu === 'upload' && (
                    ...
                    {!showJDInput ? (
                      ...
                      <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 12 }}>
                        ...
                        <button
                          ...
                          style={{ flex: 1 }}
                        >
                          {jdLoading ? 'JOBIZIC 분석 중...' : selectedTemplateId ? '분석하기 →' : '저장 & 분석하기 →'}
                        </button>
                      </div>
                      {jdLoading && (
            ...
            {activeMenu === 'upload' && !result && (
              <>
                {/* 이력서 추가 저장 쿠폰 보유 배지 */}
                {myCoupons.filter(c => c.feature === 'storage' && c.status === 'active' && !c.used_at && (c.credits - (c.used || 0)) > 0).length > 0 && (
                  <div className="coupon-active-badge">
                    🎟 이력서 추가 저장 쿠폰 {myCoupons.filter(c => c.feature === 'storage' && c.status === 'active' && !c.used_at && (c.credits - (c.used || 0)) > 0).length}개 보유 — 추가 저장이 가능합니다
                  </div>
                )}
                ...
                <label className="consent-wrap">
                  ...
                  <span className="consent-text">
                    이력서 내 직무 정보가 JOBIZIC 분석에 활용됨에 동의합니다.
          ...
          <div className="career-cards">
            ...
            {paths.length === 1 && (['RECOMMENDED', 'STRETCH'] as const).map((type) => (
              ...
              >
                ...
                <div className="career-card-title career-blur-text">JOBIZIC 추천 경로</div>
              ...
              <div className="career-lock-preview">
                ...
                <div className="career-lock-preview-body">
                  <div className="career-lock-blur-title career-blur-text">
                    {lockedTab === 'RECOMMENDED' ? 'JOBIZIC 헤드헌터 추천 직무명 (잠금됨)' : '고성장 목표 포지션 (잠금됨)'}
                  </div>
                ...
                {isPro ? (
                  ...
                  <>
                    ...
                    <div className="career-lock-cta-title">
                      {lockedTab === 'RECOMMENDED' ? 'JOBIZIC 추천 커리어 전환 경로' : '고성장 도전 커리어 경로'}
                    </div>
                    <p className="career-lock-cta-desc">
    ...
    <div className="jd-results">
      ...
      <div className="jd-ai-notice">
        ...
        <div className="jd-ai-notice-content">
          <div className="jd-ai-notice-title">JOBIZIC 분석의 특성</div>
